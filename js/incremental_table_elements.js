...
function ($scope, $document, $interval, $sce, $filter, $timeout) {
  ...
  $scope.elementPrice = function (element) {
    return Math.pow($scope.player.elements_unlocked + 1,
        $scope.elements[element].order);
  };
  ...
  $scope.generatorPrice = function (name, element) {
    ...
    var price = $scope.generators[name].price
        * Math.pow($scope.generators[name].priceIncrease, level);
    return Math.ceil(price);
  };

  $scope.synthesisMultiplier = function (element, synthesis) {
    var level = $scope.player.synthesis[synthesis].number;
    return Math.ceil(Math.pow($scope.synthesis_price_increase,
        level));
  };

  $scope.synthesisPower = function (element, synthesis) {
    var level = $scope.player.synthesis[synthesis].active;
    return Math.ceil(Math.pow(level,
        $scope.synthesis_power_increase));
  };
  ...
  $scope.isReactionCostMet = function (number, reaction) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      var required = Number
          .parseFloat((number * reaction.reactant[keys[i]])
              .toFixed(4));
  ...
  $scope.react = function (number, reaction) {
    ...
    if ($scope.isReactionCostMet(number, reaction)) {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        $scope.player.resources[keys[i]].number = Number
            .parseFloat($scope.player.resources[keys[i]].number
                .toFixed(4));
      }
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        $scope.player.resources[keys[i]].number = Number
            .parseFloat((current + produced).toFixed(4));
  ...
  $scope.save = function () {
    localStorage.setItem("playerStoredITE", JSON
        .stringify($scope.player));
    var d = new Date();
  ...
  $scope.load = function () {
    try {
      $scope.player = JSON.parse(localStorage
          .getItem("playerStoredITE"));
    } catch (err) {
  ...
  self.simulateDecay = function (number, half_life) {
    ...
    if (decay_per_second < 5) {
      ...
      production = Math.round(numberGenerator.nextGaussian() * std
          + mean);
    }
    return production
  }
  ...
  self.update = function () {
    ...
    for (var i = 0; i < $scope.radioisotopes.length; i++) {
      ...
      if ($scope.player.resources[radioisotope].unlocked) {
        ...
        // produce energy
        if ($scope.resources[radioisotope].decay.decay_energy
            * production > 0) {
          $scope.player.resources["energy"].number += Number
              .parseFloat(($scope.resources[radioisotope].decay.decay_energy * production)
                  .toFixed(4));
          $scope.$emit("resource", "energy");
          $scope.$emit("decay",
              $scope.resources[radioisotope].decay.decay_type);
        }
        // and decay products
    ...
    for (var i = 0; i < $scope.free_radicals.length; i++) {
      ...
      if ($scope.player.resources[radical].unlocked) {
        ...
        var variance = number * p * q;
        var std = Math.sqrt(variance);
        production = Math.round(numberGenerator.nextGaussian()
            * std + mean);
        ...
        for (var j = 0; j < $scope.resources[radical].free_radical.reaction.length - 1; j++) {
          ...
          if (reactants_number == 0) {
            ...
            var p = $scope.player.resources[reaction.reactant].number
                / reactants_number;
          }
          ...
          var std = Math.sqrt(variance);
          reacted[product] = Math.round(numberGenerator
              .nextGaussian()
              * std + mean);
          ...
          if (reaction.reactant == radical) {
            var adjusted_production = Math
                .floor(reacted[product] / 2);
            $scope.player.resources[radical].number += reacted[product]
                % 2 + adjusted_production;
    ...
    for ( var element in $scope.player.elements) {
      ...
      for (var i = 0; i < isotopes.length - 1; i++) {
        ...
        var p = $scope.resources[isotopes[i]].ratio
            / remaining_ratio_sum;
        var q = 1 - p;
  ...
  $scope.reactionFormat = function (number, reaction) {
    var reactionHTML = "";
    reactionHTML += $scope
        .compoundFormat(number, reaction.reactant);
  ...
  $scope.compoundFormat = function (number, compound) {
    ...
    for (var i = 0; i < keys.length; i++) {
      if (Number.isInteger(number) && number > 1) {
        compoundHTML += $scope.prettifyNumber(Number
            .parseFloat((number * compound[keys[i]]).toFixed(4)))
            + " ";
  ...
  $scope.decayFormat = function (decay) {
    ...
    if (decay.decay_energy) {
      format += " + " + $scope.prettifyNumber(decay.decay_energy)
          + ' ' + $scope.getHTML('energy');
    }
    return format;
  }
  ...
  $scope.prettifyNumber = function (number) {
    ...
    if (number > 1e6) {
      ...
      return $filter('number')(exponential[0]) + " &#215; 10<sup>"
          + $scope.prettifyNumber(exponent) + "</sup>";
    }
  ...
  self.initializeListeners = function () {
    for ( var key in $scope.unlocks) {
      if (!$scope.player.unlocks[key]) {
        $scope.unlocks[key].listener = $scope.$on(
            $scope.unlocks[key].event, $scope.unlocks[key].check);
      }
    }
  };
  ...
  $scope.updateTheme = function () {
    document.getElementById('theme_css').href = 'styles/'
        + $scope.player.current_theme + '-bootstrap.min.css';
  };
